...
gcloud compute ssh --ssh-key-file ~/.ssh/cloud-computing ubuntu@$MEMCACHED_VM --zone=$ZONE --command "echo 'running memcahed_init.sh' && bash memcached_init.sh"

echo "[Log] Installing mcperf on $CLIENT_AGENT_VM"
gcloud compute scp ./mcperf_init.sh ubuntu@$CLIENT_AGENT_VM:~ --zone=$ZONE
gcloud compute ssh --ssh-key-file ~/.ssh/cloud-computing ubuntu@$CLIENT_AGENT_VM --zone=$ZONE --command "echo 'running mcperf_init.sh' && bash mcperf_init.sh"

echo "[Log] Installing mcperf on $CLIENT_MEASURE_VM"
gcloud compute scp ./mcperf_init.sh ubuntu@$CLIENT_MEASURE_VM:~ --zone=$ZONE
gcloud compute ssh --ssh-key-file ~/.ssh/cloud-computing ubuntu@$CLIENT_MEASURE_VM --zone=$ZONE --command "echo 'running mcperf_init.sh' && bash mcperf_init.sh"
